#include "query.hh"
lgx::db::mysql::sql *lgx::data::mysql::sql = nullptr;
lgx::thread::mutex_lock lgx::data::mysql::lock;

lgx::db::mysql::query::query() :
    res_(nullptr),
    mysql_(lgx::data::mysql::sql->get()) {
}

void lgx::db::mysql::query::query::operator=(query &q) {
    row_ = q.row_;
    res_ = q.res_;
...
bool lgx::db::mysql::query::exec(std::string sql) {
    int ret = true;
...
bool lgx::db::mysql::query::select(const std::string &table_name, const std::string &key_sql, const std::string &condition) {
    std::string sql;
    sql = "SELECT " + key_sql;
    sql += " FROM " + table_name;
    if(condition != "none")
...
bool lgx::db::mysql::query::insert(const std::string &table_name, const std::string &key_sql, const std::string &value_sql) {
...
bool lgx::db::mysql::query::update(const std::string &table_name, const std::string &key_sql, const std::string &value_sql, const std::string &condition) {
    lgx::thread::mutex_lock_guard mutex_lock_guard(lgx::data::mysql::lock);
    std::string sql;
...
bool lgx::db::mysql::query::remove(const std::string &table_name, const std::string &condition) {
    lgx::thread::mutex_lock_guard mutex_lock_guard(lgx::data::mysql::lock);
    std::string sql;
...
const char* lgx::db::mysql::query::value(int index) {
...
void lgx::db::mysql::query::set_mysql(MYSQL *mysql) {
    mysql_ = mysql;
}
MYSQL *lgx::db::mysql::query::get_mysql() {
    return mysql_;
}

void lgx::db::mysql::query::escape(std::string &str) {
   std::string str_ret;
   for(auto c : str) {
       if(c == '\'')
           str_ret += "\\'";
